...
export function RoleSwitcher({
  ...
  const settingsRolesHref = "/settings?tab=profile#my-roles";
  ...
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            className={cn(
              ...
              isCompact ? "max-w-[11rem] px-3 text-sm" : "max-w-[13rem] text-base",
              className
            )}
        ...
        <DropdownMenuContent
          align="end"
          className="w-[min(100vw-2rem,20rem)] rounded-xl border-border/80 p-1 shadow-lg dark:border-gray-800 dark:bg-gray-900"
        >
          ...
          {hasLister ? (
            <DropdownMenuItem
              ...
              className={cn(
                "cursor-pointer gap-3 rounded-lg py-3 pl-3 pr-2 focus:bg-sky-50 data-[disabled]:opacity-100 dark:focus:bg-sky-900/25",
                activeRole === "lister" && "bg-sky-50/80 dark:bg-sky-900/20"
              )}
            >
              <Home className="h-5 w-5 shrink-0 text-sky-600 dark:text-sky-400" aria-hidden />
              <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                <span className="font-semibold text-sky-900 dark:text-sky-100">Lister</span>
                <span className="text-xs text-muted-foreground">Post listings &amp; hire cleaners</span>
              </div>
              {activeRole === "lister" ? (
                <Badge className="shrink-0 border-sky-300 bg-sky-100 text-[11px] font-semibold text-sky-900 dark:border-sky-700 dark:bg-sky-900/60 dark:text-sky-100">
                  Current
                </Badge>
              ) : (
            ...
            <DropdownMenuItem asChild className="cursor-pointer gap-3 rounded-lg py-3">
              <Link href={settingsRolesHref} className="flex items-center gap-3">
                <Home className="h-5 w-5 shrink-0 text-sky-600" aria-hidden />
                <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                  <span className="font-semibold text-sky-900 dark:text-sky-100">Add Lister role</span>
                  <span className="text-xs text-muted-foreground">Single login — add in Settings</span>
                </div>
              </Link>
            </DropdownMenuItem>
          )}
          ...
          {hasCleaner ? (
            <DropdownMenuItem
              ...
              className={cn(
                "cursor-pointer gap-3 rounded-lg py-3 pl-3 pr-2 focus:bg-emerald-50 data-[disabled]:opacity-100 dark:focus:bg-emerald-900/20",
                activeRole === "cleaner" && "bg-emerald-50/80 dark:bg-emerald-900/20"
              )}
            >
              <Brush className="h-5 w-5 shrink-0 text-emerald-600 dark:text-emerald-400" aria-hidden />
              <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                <span className="font-semibold text-emerald-900 dark:text-emerald-100">Cleaner</span>
                <span className="text-xs text-muted-foreground">Bid on jobs &amp; get paid</span>
              </div>
              {activeRole === "cleaner" ? (
                <Badge className="shrink-0 border-emerald-300 bg-emerald-100 text-[11px] font-semibold text-emerald-950 dark:border-emerald-700 dark:bg-emerald-900/60 dark:text-emerald-100">
                  Current
                </Badge>
              ) : (
                <Badge variant="outline" className="shrink-0 text-[11px]">
            ...
            <DropdownMenuItem asChild className="cursor-pointer gap-3 rounded-lg py-3">
              <Link href={settingsRolesHref} className="flex items-center gap-3">
                <Brush className="h-5 w-5 shrink-0 text-emerald-600" aria-hidden />
                <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                  <span className="font-semibold text-emerald-900 dark:text-emerald-100">Add Cleaner role</span>
                  <span className="text-xs text-muted-foreground">ABN required — add in Settings</span>
                </div>
              </Link>
            </DropdownMenuItem>
          )}
      ...
      {toast.visible && (
        <div className="pointer-events-none fixed bottom-24 left-1/2 z-[60] -translate-x-1/2 rounded-full bg-background/95 px-4 py-2 text-sm font-medium shadow-lg ring-1 ring-border md:bottom-8 dark:bg-gray-900">
          {toast.message}
        </div>
      )}
    </>
