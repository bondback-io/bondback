-- Last run / result snapshot for notification crons (admin report).
...
COMMENT ON COLUMN public.global_settings.notification_cron_run_status IS
  'JSON map: new_listing_reminders | daily_browse_jobs_nudge → { last_run_at, ok, error, result_summary }';
