...
export function DisputeOpened({ jobId, messageText }: DisputeOpenedProps) {
  ...
  return (
    <EmailLayout
      preview={`Dispute update on Job #${jobId}`}
      viewJobUrl={viewJobUrl}
      viewJobLabel="View Job"
    >
      <Section>
        <Text style={title}>Dispute update on Job #{jobId}</Text>
        <Text style={body}>{messageText}</Text>
        <Text style={subtext}>Respond with your evidence and message in the job to resolve the dispute.</Text>
      </Section>
    </EmailLayout>
  );
}
